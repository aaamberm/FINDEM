...
```{r}
data_ts5 <- data_clean %>% mutate(Hb = Hb/0.6206)
...
data.forestplot <- bind_rows(datalist) %>% mutate(term = ordered(term, levels = order)) %>% mutate(sex = dplyr::recode(sex, "M" = "Male", "postF" = "Postmenopausal females", "preF" = "Premenopausal females"), outcome = dplyr::recode(outcome, "ferritin" = "Ferritin", "hb" = "Hemoglobin", "hbdef" = "Hb based \ndeferral", "ID" = "Iron \ndeficiency", "returnOT" = "Return"),  significant = as.factor(ifelse(p.value >= (0.05/6), 1, 0)), significant = ordered(significant, levels = c("1", "0")))
...
p1 <- ggplot(data.forestplot[data.forestplot$outcome=="Ferritin" | data.forestplot$outcome == "Hemoglobin",], aes(x=term, y=estimate, shape = significant)) +
  ...
  geom_hline(yintercept = 0, color = "#ff9700", size = 0.35) +
  ...
  geom_point() + coord_flip() + scale_x_discrete(breaks = c("Intervention_months_factor1", "Intervention_months_factor2", "Intervention_months_factor3", "Intervention_months_factor4", "Intervention_months_factor5", "Intervention_months_factor6"),
                           labels = c("6-11 months", "12-17 months", "18-23 months", "24-29 months", "30-35 months", "36+ months")) + ylab("Regression coefficient") + facet_grid(outcome~sex) + theme(legend.position = "none",axis.title.y=element_blank())
...
p2 <- ggplot(data.forestplot[data.forestplot$outcome=="Iron \ndeficiency" | data.forestplot$outcome == "Hb based \ndeferral" | data.forestplot$outcome == "Return",], aes(x=term, y=estimate, shape = significant)) +
         geom_errorbar(aes(ymin=conf.low, ymax=conf.high), 
                       width = 0.2,size  = 0.5,
                       position = "dodge", color="#1cdbfc") +
  geom_hline(yintercept = 1, color = "#ff9700", size = 0.35) +
  scale_shape_manual(values = c(1, 16), name = "Statistical \nsignificance          ", labels = c("No", "Yes")) +
  geom_point() + coord_flip() + scale_x_discrete(breaks = c("Intervention_months_factor1", "Intervention_months_factor2", "Intervention_months_factor3", "Intervention_months_factor4", "Intervention_months_factor5", "Intervention_months_factor6"),
                           labels = c("6-11 months", "12-17 months", "18-23 months", "24-29 months", "30-35 months", "36+ months")) + ylab("Odds ratio")  + facet_grid(outcome~sex) + theme(legend.key.size = unit(0.5, 'cm'), #change legend key size
...
annotate_figure <- annotate_figure(figure,
                ...
                bottom = text_grob("Reference group: no implementation and 0-5 months since implementation. \nAll analyses are adjusted for age, height and weight. \nReturn is additionally adjusted for whether return would be during COVID pandemic.", color = "black",
                                   hjust = 0, x = 0, size = 8),
                left = text_grob("Months after intervention", color = "black", rot = 90, vjust=1, x=0.1)
                )
ggsave(annotate_figure, file = "forestplot.jpg")
```