...
data_clean$stap2 <- recode(data_clean$stap2, `2.1`="2",`2.2`="2", `3.1`="3", `3.2`="3")

data_clean$Ndonation <- data_clean$TotaalOpBaseline
data_clean$Ndonation[data_clean$Ndonation>0] <- 1
data_clean$Ndonation <- recode(data_clean$Ndonation, `1`="0",`0`="1")
data_clean$Ndonation <- as.factor(data_clean$Ndonation)
```

```{r}
#for pre menopausal females
prefemales <- subset(data_clean, data_clean$Gender=="Female" & data_clean$PostMeno==0)
prefemalest1<-table1(~ Age + Hb + Ferritin + Height + Weight +TotaalOpBaseline + Previous2yr + factor(IronDef) + factor(Return) + factor(Ndonation) | factor(stap2), data = prefemales, caption = "Females", render.continuous=c(.="Mean (SD)", .="Median [IQR]"), render.categorical=c(.="FREQ (PCT)"))

#for post menopausal females
postfemales <- subset(data_clean, data_clean$Gender=="Female" & data_clean$PostMeno==1)
postfemalest1<-table1(~ Age + Hb + Ferritin + Height + Weight +TotaalOpBaseline + Previous2yr + factor(IronDef) + factor(Return) + factor(Ndonation) | factor(stap2), data = postfemales, caption = "Females", render.continuous=c(.="Mean (SD)", .="Median [IQR]"), render.categorical=c(.="FREQ (PCT)"))

#for males
males <- subset(data_clean, data_clean$Gender=="Male")
malest1<- table1(~ Age + Hb + Ferritin + Height + Weight +TotaalOpBaseline + Previous2yr + factor(IronDef) + factor(Return) + factor(Ndonation) | factor(stap2), data = males, caption = "Males", render.continuous=c(.="Mean (SD)", .="Median [IQR]"), render.categorical=c(.="FREQ (PCT)"))
