...
data_clean <- data_raw2 %>% 
  ...
  rename(Ferritin = Ferritine_FINDEM, Age = LeeftijdBijDOnatie, Cluster = cluster, Weight = Gewicht, Height = Lengte)  %>% mutate(Ferritingood = factor(case_when(Ferritin > 15 ~ 1, Ferritin < 15 ~ 0)))%>% select(-HbGoedgekeurd, -Geslacht)%>%
...
hbdef_fitFfactor <- update(hbdef_fitF, formula = Hbgood ~ Age + Weight + Height + Intervention_months_factor)
...
hbdef_fitMfactor <- update(hbdef_fitM, formula = Hbgood ~ Age + Weight + Height + Intervention_months_factor)
...
```{r ferritin good females}
ferrgood_fitF <- glm(formula = Ferritingood ~ Age + Weight + Height + Intervention_months, data = data_scale, subset = data_scale$Gender == "Female",family = binomial)

ferrgood_fitFfactor <- update(ferrgood_fitF, formula = Ferritingood ~ Age + Weight + Height + Intervention_months_factor)
ferrgood_fit_preF_factor <- update(ferrgood_fitF, formula = Ferritingood ~ Age + Weight + Height + Intervention_months_factor, subset = data_scale$Gender == "Female" & data_scale$PostMeno == 0)
ferrgood_fit_postF_factor <- update(ferrgood_fitF, formula = Ferritingood ~ Age + Weight + Height + Intervention_months_factor, subset = data_scale$Gender == "Female" & data_scale$PostMeno == 1)

```

```{r ferritin good males}
ferrgood_fitM <- glm(formula = Ferritingood ~ Age + Weight + Height + Intervention_months, data = data_scale, subset = data_scale$Gender == "Male",family = binomial)

ferrgood_fitMfactor <- update(ferrgood_fitF, formula = Ferritingood ~ Age + Weight + Height + Intervention_months_factor)
...
FileName <- paste("HTML tables/", currentDate, "/ferrgood_fit.html",sep="")
tab_model(ferrgood_fit_preF_factor, ferrgood_fit_postF_factor, ferrgood_fitMfactor, file = FileName, pred.labels = pl, dv.labels = c("Pre-menopausal women", "Post-menopausal women", "Men"), title = "Ferritin levels above 15", show.reflvl = T)
